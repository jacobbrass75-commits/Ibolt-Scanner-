...
Open **https://inventory.89.167.10.34.nip.io** in Chrome or Safari and choose **Continue with Google** to create an account or sign in. If the link opens inside an email app, open it in the browser first. **Email verification codes are currently unavailable:** on September 15, Clerk's delivery logs confirmed sender-authentication failures for the `nip.io` mail domain. Google sign-in remains the verified route; email/password signup must not be treated as working until mail delivery is repaired and tested.

On this configured PC, double-click **Start Inventory.cmd** to open the same hosted app. Authentication uses Clerk. The original live **iBolt Inventory** application now belongs to Jacob's **Ibolt** organization, with open registration saved and the allowlist disabled. Release `e1febb0` added public app/privacy pages alongside Google sign-in and bounded sign-in recovery. Google sign-in and repeat sign-in were verified against the live inventory. Google may display `nip.io` in its account chooser because that is the current hostname. See [docs/GOOGLE-SIGN-IN.md](docs/GOOGLE-SIGN-IN.md) for the current authentication checkpoint. The existing Hetzner server is reused at no added server cost. HTTPS, automatic restart, hourly verified backups, and recovery are in [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md).
