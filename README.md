...
Open **https://iboltscan.com** in Chrome or Safari. Choose **Continue with Google**, or use your email address. If asked for a code, check your inbox and spam folder. If the link opens inside an email app, open it in the browser first. On September 15, all five Clerk DNS records were verified, including all three email records. Real email-code delivery, email-code sign-in, and Google sign-in were then verified on the new domain with the owner's existing administrator access. Registration is open; creating a previously unregistered person's account has not been tested end to end. See [docs/GOOGLE-SIGN-IN.md](docs/GOOGLE-SIGN-IN.md).

On this configured PC, double-click **Start Inventory.cmd** to open the same hosted app. Authentication uses Clerk. The original **iBolt Inventory** production application remains in Jacob's **Ibolt** organization, with open registration and the allowlist disabled. The domain migration preserves this instance and its accounts. Release `e1febb0` added public app/privacy pages alongside Google sign-in and bounded sign-in recovery. Google login was verified on the former `nip.io` address; its email codes failed sender authentication. See [docs/GOOGLE-SIGN-IN.md](docs/GOOGLE-SIGN-IN.md) for the current authentication checkpoint and [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md) for the old-link redirect, service, and backup setup. The existing Hetzner server is reused at no added server cost.
